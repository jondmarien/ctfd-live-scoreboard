...
export default function SolvesPanel({ challengeId }: SolvesPanelProps) {
  ...
  if (solves.length === 0) {
    return (
      <section className="mb-6 rounded-lg border border-amber-800/30 bg-stone-900/30 p-4">
        <p className="font-body text-sm text-amber-500/60 italic">
  ...
  return (
    <section className="mb-6 rounded-lg border border-amber-800/30 bg-stone-900/30 p-4">
      <h3 className="mb-2 font-display text-base text-amber-200">
        {solves.length} {solves.length === 1 ? "Victor" : "Victors"}
      </h3>
      <ul className="space-y-1 text-sm">
        {solves.slice(0, 10).map((solve, index) => (
          <li key={`${solve.account_id}-${solve.date}`} className="flex justify-between font-body text-amber-300/80">
      ...
      {solves.length > 10 && (
        <p className="mt-2 font-body text-xs text-amber-500/60">
